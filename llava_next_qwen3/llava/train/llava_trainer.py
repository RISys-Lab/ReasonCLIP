...
class LLaVATrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self._accumulated_step_time = 0.0  # Accumulated time for gradient accumulation steps
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            dataloader_params["worker_init_fn"] = partial(
                seed_worker, num_workers=self.args.dataloader_num_workers, rank=self.args.local_rank
            )
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        """
        Perform a training step on a batch of inputs and log the step time.
        Considers gradient accumulation: accumulates time for all micro-batches.
        """
        ...
        # Accumulate time for each micro-batch
        self._accumulated_step_time += time.time() - step_start_time

        return loss

    def log(self, logs, start_time=None):
        """
        Override log to add step_time to the same log entry as loss.
        Output accumulated time when logging (includes all gradient accumulation steps).
        """
        if self._accumulated_step_time > 0 and "loss" in logs:
            logs["step_time"] = round(self._accumulated_step_time, 1)
            logs["step"] = self.state.global_step
            self._accumulated_step_time = 0.0  # Reset accumulated time
