...
# 读取parquet文件
file_path = "/home/muzammal/Projects/CLIP-R/data/fesvhtr-CLIPReasonItw/itw_neg.parquet"
...
def extract_captions(text):
    """从格式为 '1.xxx 2.xxx' 的文本中提取两个caption"""
    if pd.isna(text):
        return None
    ...
    if len(matches4) >= 2:  # 至少有两组caption
        # 按数字分组
        groups = {}
        ...
        if len(groups.get(1, [])) > 1 and len(groups.get(2, [])) > 1:
            captions4 = [
    ...
    if len(lines5) >= 2:
        ...
        if not any(re.match(r'^\d+\.', line) for line in lines5):
            captions5 = [re.sub(r'\s+', ' ', line) for line in lines5[:2]]  # 只取前3个
            return captions5
...
# 应用到整个数据框
df['trp_neg'] = df['generated_text'].apply(extract_captions)
# df['processed_text'] = df['generated_text'].apply(get_processed_text)

# 统计成功和失败的情况
success_rows = df[df['trp_neg'].notna()]  # 成功提取的行
failed_rows = df[df['trp_neg'].isna()]    # 失败的行（None）
...
caption_counts = df['trp_neg'].apply(lambda x: len(x) if x is not None else 0).value_counts().sort_index()
...
if len(failed_rows) > 0:
    ...
    for i, (idx, row) in enumerate(failed_rows.head(10).iterrows()):
        ...
        print(f"处理结果: {row['trp_neg']}")
        print("-" * 80)


output_path = "/home/muzammal/Projects/CLIP-R/data/fesvhtr-CLIPReasonItw/itw_neg_cleaned.parquet"
