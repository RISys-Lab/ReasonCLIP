#!/usr/bin/env python3
"""NYUv2 frozen-backbone surface-normal evaluation."""
...
from geometry_data import (  # noqa: E402
    NYUGeoNetDataset,
    NYUTestDataset,
    dataset_protocol,
)
...
def build_datasets(args: argparse.Namespace, limits: RunLimits):
    train_dataset = NYUGeoNetDataset(
        args.data_root / "NYUv2" / "nyuv2_geonet",
        task="normals",
        augment=True,
        max_samples=limits.max_train,
    )
    eval_dataset = NYUTestDataset(
        args.data_root / "NYUv2" / "nyuv2_snorm_all.pkl",
...
def protocol_summary(
    ...
    return {
        "benchmark": "nyuv2_normals",
        "paper_protocol": "TIPS Appendix B + Probe3D NYUv2 normals",
        ...
        "loss": "mean angular error in radians over depth > 0",
